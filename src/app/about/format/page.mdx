...
<PageHeader
  ...
  description="Specification of the standardized AHORN dataset format, including structure, metadata, and design goals."
  actions={<Badge color="info">Version 0.3</Badge>}
/>
...
```plain
{"name": "example-dataset", "more-dataset-metadata": "value", "format-version": "0.3", "revision": 1}
...
The dataset-level metadata MUST list the number of networks in the `num-networks` attribute.

```plain
{"num-networks": 2, "format-version": "0.2", "revision": 2}
...
> [!NOTE]
> For some datasets, nodes are numbered but get assigned a unique name as attribute, e.g., `{"name": "John Doe"}`. This makes the edge definitions simpler and more compact.
...
Attributes are stored as key-value pairs, where the key is a string (alphanumeric and dashes) and the value can be of arbitrary type.
...
The dataset-level metadata MUST include the `revision` attribute, which is an integer that indicates the version of the dataset.
When a dataset is updated or modified, the `revision` number MUST be incremented by one.

## Sub-datasets

Some datasets are published as sub-datasets of a larger parent dataset.
In that case, the dataset `name` MUST be the parent dataset name followed by a dash and the sub-dataset name, for example `cooking-brazilian`.

Sub-datasets MUST always use the same `revision` number as their parent dataset if they are derived from it directly.
As a consequence, a newly introduced sub-dataset MAY start at a revision number greater than `1` if the parent dataset is already at a later revision.

## Changelog

<details className="mt-3 rounded border border-black-25 bg-white p-4 dark:border-black-75 dark:bg-black-100">
  <summary className="cursor-pointer select-none">Version 0.3</summary>
  <div>
    - Reserved metadata keys do not start with an underscore (`_`) anymore, but are named without the underscore, e.g., `format-version` instead of `_format-version`.
  </div>
</details>
<details className="mt-3 rounded border border-black-25 bg-white p-4 dark:border-black-75 dark:bg-black-100">
